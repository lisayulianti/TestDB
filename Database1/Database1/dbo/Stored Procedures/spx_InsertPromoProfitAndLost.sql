...
-- Description:	Insert into tblPromoProfitAndLost
-- =============================================
CREATE PROCEDURE [dbo].[spx_InsertPromoProfitAndLost] --10361,102
	@PromotionID int,
	@CountryID int = 102
AS
BEGIN
...
if exists (select top 1 promoProfitAndLossID from tblPromoProfitAndLoss WITH (NOLOCK) where promotionid = @promotionID)
begin
	delete tblPromoProfitAndLossDetail where promotionID = @promotionID
	delete tblPromoProfitAndLoss where promotionID = @promotionID
end
...
select * 
from dbo.fn_GetProfitAndLossCriteriaByPromotion(@promotionID)
...
begin
	...
	from vw_PromoCust v WITH (NOLOCK) 
...
where bal.balYear = datepart(year, @prmStartDate)
...
left join (select sum(budGrossSales) budGrossSales, cusGroup5ID, cusGroup6ID, secGroup2ID
	...
	where budBudgetNr = @maxBudgetAllocationNr
		and budYear = datepart(year, @prmStartDate)
...
where bal.countryID = @countryID and balBudgetNr = @maxBudgetAllocationNr
	 and bal.balYear = DATEPART(year, @prmStartDate)
...
case when avg(budGrossSales) > 0
	then avg((bal.balFixedTT)) * -(select pnlValue from @tbl where pnlField = 'GrossSales' and pnlType = 1) / avg(budGrossSales)
	else 0
end
from tblBudgetAllocation bal WITH (NOLOCK) 
inner join @tblCusGroup cusgroup on 
...
case when avg(budGrossSales) > 0
	then avg((bal.balFixedTT)) * -(select pnlValue from @tbl where pnlField = 'GrossSales' and pnlType = 2) / avg(budGrossSales)
	else 0
end
from tblBudgetAllocation bal WITH (NOLOCK) 
inner join @tblCusGroup cusgroup on 
...
case when avg(budGrossSales) > 0
	then avg((bal.balFixedTT)) * -(select pnlValue from @tbl where pnlField = 'GrossSales' and pnlType = 5) / avg(budGrossSales)
	else 0
end
from tblBudgetAllocation bal WITH (NOLOCK) 
inner join @tblCusGroup cusgroup on 
...
begin
	...
	from tblProfitAndLossHelper helper WITH (NOLOCK) 
	left outer join 
	(
	select 
	...
	inner join tblLKATT att WITH (NOLOCK) on att.secGroup2ID = seccust.secGroup2ID and att.prdGroup11ID = prd.prdGroup11ID 
		and (select prmDateStart from tblPromotion where promotionID = @PromotionID) between att.validFrom and att.validTo	
...
begin
	...
	from tblPromotionProductSelection prmprd WITH (NOLOCK) 
	inner join tblProduct prd on prd.ProductID = prmprd.prdProductID
	where promotionID = @PromotionID
	) prd on
...
where finCostTypeID in (1,2) and promotionID = @promotionID
	 and helper.countryID = @CountryID	
...
from tblPromoFinancial prmfin WITH (NOLOCK) 
inner join tblProfitAndLossHelper helper WITH (NOLOCK) on helper.PnLCode = prmfin.pnlID
where finCostTypeID = 2 and promotionID = @promotionID
	 and helper.countryID = @CountryID	
group by PnLSubGroupItem, PnLSubGroup, finCostTypeID, PnlGroup
...
from tblBudgetAndForecast bud
where bud.budYear = DATEPART(year, @prmStartDate)
...
inner join @tblCusGroup cusgroup on 
	...
	--isnull(cusgroup.cusGroup5ID, '') = isnull(baf.CusGroup5ID, '') 
	--and isnull(cusgroup.cusGroup6ID, '') = isnull(baf.cusGroup6ID, '')
	--and isnull(cusgroup.secGroup2ID, '') = isnull(baf.secGroup2ID, '')
where
...
-- INSERT RESULT TO tblPromoProfitAndLoss

insert into tblPromoProfitAndLoss (promotionid, pnlitem, finCostTypeID, finAmount)
select @promotionID promotionID, helper.pnlitem, tbl.pnlType, sum(isnull(tbl.pnlValue,0))
from @tbl tbl
...
--select * from tblPromoProfitAndLoss where promotionID = @promotionID
--order by finCostTypeID, PnLItem
--select * from @tblSubItem
--order by finCostTypeID, pnlGroup, pnlItem

insert into tblPromoProfitAndLossDetail (promoProfitAndLossID, pnlSubItem, finCostTypeID, finAmount, promotionID)
select 
promopnl.promoProfitAndLossID, subitem.pnlSubItem, subitem.finCostTypeID, sum(subitem.finAmount), promopnl.promotionID
from @tblSubItem subitem
inner join tblProfitAndLossHelper helper on helper.PnLSubGroup = subitem.pnlItem and helper.PnlGroup = subitem.pnlGroup
	and helper.PnLSubGroupItem = subitem.pnlSubItem
inner join tblPromoProfitAndLoss promopnl on promopnl.pnlitem = helper.PnLItem and promopnl.finCostTypeID = subitem.finCostTypeID
where promopnl.promotionID = @promotionID
	 and helper.countryID = @CountryID	
group by promopnl.promoProfitAndLossID, subitem.pnlSubItem, subitem.finCostTypeID, promotionID
--order by subitem.finCostTypeID, helper.PnlGroup, PnLSubGroup

-- insert data to tblPromoEvaluation
exec spx_InsertPromoEvaluation @promotionID, @countryID
