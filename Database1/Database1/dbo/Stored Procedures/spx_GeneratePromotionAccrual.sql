...
CREATE procedure [dbo].[spx_GeneratePromotionAccrual]
	@Bookmonth int,
	@Bookyear int,
...
BEGIN 
	...
	select
	prmPromotionClientCode, prmPromotionBookMonth, col1, col2, finGeneralLedgerCode, finGeneralLedgerCodeDesc,
	sum(col3) col3,
	'' as col4,
	col5,
	'' as col6,
	...
	col8,
	'' as col9,
	col10, CustomerClientCode, prdProductClientCode, 
	'' as Plant,
	col12, col13, col14, promotionid, topup, combinationLedgerCode, pnlID,finSpendType,prodHierarchy,pccCostCentreDesc
	from
	(
	select
	distinct 
	*
	...
	(
	SELECT
	--jcc.jccCusCustomerClientCode, cus.cusCustomerClientCode ,
	...
	CAST(fin.finAmount 
		* (pnl.GrossSales/ tbl.totalGrossSalesPerPromo) 
		* (CASE WHEN @Firstday < prm.prmDateStart THEN
				CASE WHEN @Lastday < prm.prmDateEnd THEN
					(cast(DATEDIFF(day,prm.prmDateStart,@Lastday)+1 as DECIMAL(18,0) ) / cast(DATEDIFF(day,prm.prmDateStart,prm.prmDateEnd)+1 as DECIMAL(18,0) ))
				ELSE
					(cast(DATEDIFF(day,prm.prmDateStart,prm.prmDateEnd)+1 as DECIMAL(18,0) ) / cast(DATEDIFF(day,prm.prmDateStart,prm.prmDateEnd)+1 as DECIMAL(18,0) ))
				END
			ELSE
				CASE WHEN @Lastday < prm.prmDateEnd THEN
					(cast(DATEDIFF(day,@Firstday,@Lastday)+1 as DECIMAL(18,0) ) / cast(DATEDIFF(day,prm.prmDateStart,prm.prmDateEnd)+1 as DECIMAL(18,0) ))
				ELSE
					(cast(DATEDIFF(day,@Firstday,prm.prmDateEnd)+1 as DECIMAL(18,0) ) / cast(DATEDIFF(day,prm.prmDateStart,prm.prmDateEnd)+1 as DECIMAL(18,0) ))
				END
			END) AS DECIMAL(18,2)) as col3,
	--'' as col4,
	...
	CASE WHEN fin.finSpendType='A&P' THEN '' ELSE CAST(prd.prdProductClientCode AS VARCHAR(50)) END AS prdProductClientCode ,
	--'' as Plant,
	'2000' as col12,
	SUBSTRING(CAST(cus.cusGroup1ID AS VARCHAR(50)),8,LEN(cus.cusGroup1ID)) as col13 ,
	'10' as col14,
	prm.promotionID ,0 as topup ,
	...
	INNER JOIN tblProduct AS prd ON prd.productID = prod.prdProductID
	...
	WHERE prm.[countryID]=102 AND fin.finCostTypeID=1 AND prd.productID=prod.prdProductID
	...
	WHERE tbl.col3>0
	group by prmPromotionClientCode, prmPromotionBookMonth, col1, col2, col5, col7, col8, finGeneralLedgerCode, finGeneralLedgerCodeDesc,
	col10, CustomerClientCode, prdProductClientCode, col12, col13, col14, promotionid, topup, combinationLedgerCode, pnlID,finSpendType,prodHierarchy,pccCostCentreDesc
