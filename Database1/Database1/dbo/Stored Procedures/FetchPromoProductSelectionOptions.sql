...
-- =============================================
CREATE PROCEDURE [dbo].[FetchPromoProductSelectionOptions] 
	@countryId int 
AS
BEGIN
   ...
   SELECT p.productID,p.countryID,prdProductClientCode,prdProductName,prdProductClientCode,prdParentCode,
                    ...
                    FROM tblProduct p 
                    INNER JOIN (SELECT countryID, productID, MAX(validFrom) validFromMax FROM tblProduct GROUP BY countryID, productID) p2 
	                        ON p.countryID = p2.countryID AND p.productID = p2.productID AND p.validFrom = p2.validFromMax
					LEFT JOIN tblPrdGroup1 pg1 on p.prdGroup1ID = pg1.prdGroup1ID
					LEFT JOIN tblPrdGroup2 pg2 on p.prdGroup2ID = pg2.prdGroup2ID
					LEFT JOIN tblPrdGroup3 pg3 on p.prdGroup3ID = pg3.prdGroup3ID
					LEFT JOIN tblPrdGroup4 pg4 on p.prdGroup4ID = pg4.prdGroup4ID
					LEFT JOIN tblPrdGroup5 pg5 on p.prdGroup5ID = pg5.prdGroup5ID
					LEFT JOIN tblPrdGroup6 pg6 on p.prdGroup6ID = pg6.prdGroup6ID
					LEFT JOIN tblPrdGroup7 pg7 on p.prdGroup7ID = pg7.prdGroup7ID
					LEFT JOIN tblPrdGroup8 pg8 on p.prdGroup8ID = pg8.prdGroup8ID
					LEFT JOIN tblPrdGroup9 pg9 on p.prdGroup9ID = pg9.prdGroup9ID
					LEFT JOIN tblPrdGroup10 pg10 on p.prdGroup10ID = pg10.prdGroup10ID
					LEFT JOIN tblPrdGroup11 pg11 on p.prdGroup11ID = pg11.prdGroup11ID
					LEFT JOIN tblPrdGroup12 pg12 on p.prdGroup12ID = pg12.prdGroup12ID
					LEFT JOIN tblPrdGroup13 pg13 on p.prdGroup13ID = pg13.prdGroup13ID
					LEFT JOIN tblPrdGroup14 pg14 on p.prdGroup14ID = pg14.prdGroup14ID
					LEFT JOIN tblPrdGroup15 pg15 on p.prdGroup15ID = pg15.prdGroup15ID
					LEFT JOIN tblPrdGroup16 pg16 on p.prdGroup16ID = pg16.prdGroup16ID
					LEFT JOIN tblPrdGroup17 pg17 on p.prdGroup17ID = pg17.prdGroup17ID
					LEFT JOIN tblPrdGroup18 pg18 on p.prdGroup18ID = pg18.prdGroup18ID
					LEFT JOIN tblPrdGroup19 pg19 on p.prdGroup19ID = pg19.prdGroup19ID
					LEFT JOIN tblPrdGroup20 pg20 on p.prdGroup20ID = pg20.prdGroup20ID
					LEFT JOIN tblPrdGroup21 pg21 on p.prdGroup21ID = pg21.prdGroup21ID
					LEFT JOIN tblPrdGroup22 pg22 on p.prdGroup22ID = pg22.prdGroup22ID
